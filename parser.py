...
def main():
    ...
    for subdirectory in subdirectories:
        ...
        iterationDirectories = os.listdir(f"{directory}/{subdirectory}")
        iterationDirectories.sort(key=natural_keys)
        for iteration in iterationDirectories:

            # Jeder File in der Iteration
            filesInIteration = os.listdir(f"{directory}/{subdirectory}/{iteration}")
            filesInIteration.sort(key=natural_keys)
            for file in filesInIteration:
                ...
                if os.path.isfile(path) and not isEmpty(path):
                    ...
                    print(f"Write - File {subdirectory}.{iteration}.{os.path.basename(file)} got result '{result_white}-{result_black}'")
        ...
        iterations = os.listdir(f"{directory}/{subdirectory}")
        ...
        for iteration in iterations:
            ...
            # Jeder File in der Iteration
            files_two = os.listdir(f"{directory}/{subdirectory}/{iteration}")
            files_two.sort(key=natural_keys)
            for file in files_two:
                ...
                if not isEmpty(path):
                    ...
                    print(f"Read - File {subdirectory}.{iteration}.{os.path.basename(file)} got result '{resultsWhite[counter]}-{resultsBlack[counter]}'")
